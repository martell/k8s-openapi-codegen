	...
	for (definition_path, definition) in &spec.definitions {
		...
		match &definition.kind {
			swagger20::SchemaKind::Properties(properties) => {
				...
				let (properties, resource_api_version_kind) = {
					use std::fmt::Write;
					...
					let resource_api_version_kind = match (has_api_version, has_kind) {
						(true, true) => {
							let single_group_version_kind = single_group_version_kind.unwrap();
							if single_group_version_kind.group == "" {
								Some((std::borrow::Cow::Borrowed(&single_group_version_kind.version), &single_group_version_kind.kind))
							}
							else {
								Some((std::borrow::Cow::Owned(format!("{}/{}", single_group_version_kind.group, single_group_version_kind.version)), &single_group_version_kind.kind))
							}
						},
						(false, false) => None,
					...
					(result, resource_api_version_kind)
				};
				...
				if let Some(resource_api_version_kind) = &resource_api_version_kind {
					...
					writeln!(file, r#"        "{}""#, resource_api_version_kind.1)?;
					writeln!(file, "    }}")?;
					writeln!(file, "}}")?;
				}
				...
				if properties.is_empty() && resource_api_version_kind.is_none() {
